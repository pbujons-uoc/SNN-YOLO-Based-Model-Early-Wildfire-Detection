"""Reverse Latency (TTFS) encoded .pt files to RGB images and overlay YOLO boxes.

Usage examples:
    # Single file
    python debug/un_encoding_latency.py --pt datasets/latency_4/val/images/AoF01029.pt --labels datasets/latency_4/val/labels --out debug/recon

    # Directory (process all .pt files)
    python debug/un_encoding_latency.py --pt datasets/latency_4/val/images --labels datasets/latency_4/val/labels --out debug/recon

Notes:
- Expects .pt files shaped [T, C, H, W] (dtype uint8 or float).
- Labels should be standard YOLO .txt (class cx cy w h normalized). If not provided, tries to find a labels directory by replacing 'images' with 'labels'.
- Outputs PNG files to --out directory with name <orig>_recon.png.
"""
...
def recon_latency(spikes: torch.Tensor) -> np.ndarray:
    """Reconstruct image from latency-encoded spikes.
    ...
    # ensure float
    s = spikes.float()
    # Each pixel/time has exactly one firing time in latency encoding
    # Find time index where spike==1
    times = torch.argmax(s, dim=0).float()  # [C, H, W]
    # intensity normalized: 1 - t/(T-1)
    if T <= 1:
        intensity = (times * 0.0)  # degenerate
    else:
        intensity = 1.0 - (times / (T - 1.0))
    # clamp
    intensity = intensity.clamp(0.0, 1.0)
    img = (intensity * 255.0).permute(1, 2, 0).cpu().numpy().astype(np.uint8)  # H,W,C
    return img
...
def draw_boxes(img: np.ndarray, boxes, thickness=2):
    ...
    for b in boxes:
        ...
        cv2.putText(out, text, (x1 + 3, y1 - 4), cv2.FONT_HERSHEY_SIMPLEX, 0.6, (0, 0, 0), 1, cv2.LINE_AA)
    return out
...
def process_file(pt_path: Path, labels_dir: Path, out_dir: Path):
    try:
        ...
        return
    # spikes expected [T,C,H,W]
    if isinstance(spikes, torch.Tensor) is False:
        spikes = torch.tensor(spikes)
    if spikes.ndim == 5:  # maybe [B, T, C, H, W]
        spikes = spikes.squeeze(0)
    if spikes.ndim == 3:  # maybe [C,H,W] treat as single-frame intensity
        # Convert to [T=1, C, H, W]
        spikes = spikes.unsqueeze(0)
    T, C, H, W = spikes.shape
    img = recon_latency(spikes)
    # find label file
...
def main():
    parser = argparse.ArgumentParser(description='Un-encode latency .pt spike files and overlay YOLO boxes')
    ...
    parser.add_argument('--out', default='debug/recon_latency', help='Output directory')
    ...
    if args.labels:
        ...
        # try heuristics: replace 'images' with 'labels' in path
